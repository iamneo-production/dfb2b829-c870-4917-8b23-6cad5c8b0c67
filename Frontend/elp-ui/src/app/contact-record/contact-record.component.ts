...
export class ContactRecordComponent {
  ...
  getAllContact()
  {
    this.http.get("http://localhost:8080/api/v1/contact/getAllContact")
  ...
  contact(){
   ...
   this.http.post("http://localhost:8080/api/v1/contact/save",bodyData).subscribe((resultData: any)=>
   {
     ...
     setDelete(data: any){
      this.http.delete("http://localhost:8080/api/v1/contact/delete"+"/"+data.studentid,{responseType: 'text'}).subscribe((resultData: any)=>
      {
        console.log(resultData);
        alert("Record Delete")
        this.getAllContact();
